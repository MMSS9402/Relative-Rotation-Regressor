...
def train(gpu, args):
    ...
    model = build(cfg)
    wandb.watch(model)
    # checkpoint = torch.load("/home/moon/source/CuTi/checkpoint/031932.pth")
    ...
    if args.ckpt is not None:
        ...
        if args.no_ddp:
            ...
            state_dict = OrderedDict(
                [
                    (k.replace("module.", ""), v)
                    for (k, v) in existing_ckpt["model"].items()
                ]
            )
    ...
    while should_keep_training:
        ...
        db = dataset_factory(
            ...
            gpu=gpu,
            streetlearn_interiornet_type=args.streetlearn_interiornet_type,
...
if __name__ == "__main__":
    ...
    PATHS = [
        "output/%s/checkpoints" % (args.name),
        "output/%s/runs" % (args.name),
        "output/%s/train_output/images" % (args.name),
    ]
