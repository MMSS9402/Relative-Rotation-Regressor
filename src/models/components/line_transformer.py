...
class LineTransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 max_keylines: int,
                 descriptor_dim: int,
                 keyline_encoder : List[int],
                 n_heads: int,
                 n_line_descriptive_layers: int,
                 d_inner: int):
        super().__init__()
        ...
        self.keyline_encoder = list(keyline_encoder)
        self.n_heads = n_heads
        self.n_line_descriptive_layers = n_line_descriptive_layers
        self.d_inner = d_inner
        ## Line Descriptive Network
        self.klenc = KeylineEncoder(
            self.descriptor_dim, self.keyline_encoder, self.n_heads, self.n_line_descriptive_layers, self.d_inner)

        ## Line Signature Network
        self.selfattn = SelfAttentionalLayer(self.descriptor_dim, ['self','self','self','self','self','self','self'])
    ...
    def forward(self, data, img_idx):
        if len(data['klines' + str(img_idx)])==0:
            return self.default_ret()

        klines = data['sublines'+ str(img_idx)]
        resp = data['resp_sublines'+ str(img_idx)]
        angle =  data['angle_sublines'+ str(img_idx)]
        pnt_sublines = data['pnt_sublines'+ str(img_idx)]
        desc_sublines = data['desc_sublines'+ str(img_idx)]
        score_sublines = data['score_sublines'+ str(img_idx)]
        mask_sublines = data['mask_sublines'+ str(img_idx)]
        ...
        data.update({'line_desc'+ str(img_idx): line_desc_})
    ...
    def preprocess(self, klines_cv, image_shape, pred_superpoint, valid_mask=None):
        ...
        # remove_borders
        _,_,height, width = self.image_shape = image_shape
        ...
        # filter_by_length
        klines = filter_by_length(klines, self.min_length, self.max_keylines) # 15 msec
        num_klines = len(klines['klines'])
        if num_klines == 0:
            return klines
        
        # line_tokenizer
        klines = line_tokenizer(klines, self.token_distance, self.max_tokens, pred_superpoint, image_shape[-2:])
    ...
    def subline2keyline(self, distance_sublines, mat_klines2sublines0, mat_klines2sublines1):
        ...
        mat_klines2sublines0 = mat_klines2sublines0.cpu().numpy()
        mat_klines2sublines1 = mat_klines2sublines1.cpu().numpy()
