...
class MLP(torch.nn.Module):
    def __init__(self, in_channels, out_channels, batch_norm):
        ...
        self.conv3 = nn.Conv1d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
...
def conv_1x1(in_feats, out_feats, kernel_size=3, stride=1, padding=1, dilation=1, batch_norm=False):
    if batch_norm:
        ...
        return nn.Sequential(
            nn.Conv1d(in_feats, out_feats, kernel_size=kernel_size, stride=stride,
                      padding=padding, dilation=dilation, bias=True),
            nn.ReLU())